...
def delete_meeting(slug: str) -> dict:
    ...
    try:
        ...
        return {"ok": False, "msg": "Permission denied — cannot delete meeting note"}
    except OSError as e:
        print(f"[ERROR] delete_meeting({slug}): {e}")
        return {"ok": False, "msg": f"Failed to delete meeting note: {e}"}
    return {"ok": True}
